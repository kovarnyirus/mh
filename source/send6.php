<?php
$name = htmlspecialchars($_REQUEST['name'], ENT_QUOTES);
$phone = htmlspecialchars($_REQUEST['phone'], ENT_QUOTES);
$typeOfBusiness= htmlspecialchars($_REQUEST['typeOfBusiness'], ENT_QUOTES);
$problem = htmlspecialchars($_REQUEST['problem'], ENT_QUOTES);
$runAds = htmlspecialchars($_REQUEST['runAds'], ENT_QUOTES);
$site = htmlspecialchars($_REQUEST['site'], ENT_QUOTES);
$soc = htmlspecialchars($_REQUEST['soc'], ENT_QUOTES);


$thanks_file = 'success.php';



if (!$errors) {
    $to = 'marketing.house@yandex.ru';
    $header = 'MIME-Version: 1.0'."\r\n";
    $header .= 'Content-type: text/html; charset=utf-8'."\r\n";
    $header .= 'From: =?utf-8?b?'.base64_encode("Заявка с сайта marketinghouse").'?= <no-reply@'.$_SERVER['HTTP_HOST'].'>'."\r\n";
    $header .= 'Заявка с сайта marketinghouse';

    $subject = 'Заявка с сайта marketinghouse Quiz';


    $msg = '<html>'."\n";
    $msg .= ' <head>'."\n";
    $msg .= ' <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'."\n";
    $msg .= ' </head>'."\n";
    $msg .= ' <body>'."\n";
    $msg .= ' <h2>Заявка Quiz</h2>'."\n";
    $msg .= ' <table cellpadding="5" cellspacing="0">'."\n";
    $msg .= ' <tr><td><strong>Имя:</strong></td><td>'.$name.'</td></tr>'."\n";
    $msg .= ' <tr><td><strong>Телефон:</strong></td><td>'.$phone.'</td></tr>'."\n";
    $msg .= ' <tr><td><strong>Каким видом бизнеса занимаетесь:</strong></td><td>'.$typeOfBusiness.'</td></tr>'."\n";
    $msg .= ' <tr><td><strong>Какую проблему на данный момент хотите решить:</strong></td><td>'.$problem.'</td></tr>'."\n";
    $msg .= ' <tr><td><strong>Запускали рекламу в интернете:</strong></td><td>'.$runAds.'</td></tr>'."\n";
    $msg .= ' <tr><td><strong>Есть сайт вашего бизнеса:</strong></td><td>'.$site.'</td></tr>'."\n";
    $msg .= ' <tr><td><strong>Ведёте социальные сети для бизнеса:</strong></td><td>'.$soc.'</td></tr>'."\n";
    $msg .= ' </table>'."\n";
    $msg .= ' </body>'."\n";
    $msg .= '</html>'."\n";

    if (mail($to, $subject, $msg, $header, '-fno-reply@'.$_SERVER['HTTP_HOST'])) {
        header('Location: ' . $thanks_file);
    } else {
        $output = '<p>Произошла ошибка, пожалуйста повторите попытку позже.</p>';
    }
}

print $output;
